...
export default function TherapistManager({
  ...
  const handleAddTherapist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`성공: '${newTherapistName}' 마사지사가 등록되고 로그인 계정이 발급되었습니다! (초기 비번: ${password})`)
      ...
      console.error(err)
      setErrorMsg(err.message || '마사지사 등록에 실패했습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleTogglePremiumTarget = async (id: number, currentTarget: boolean) => {
    ...
    try {
      ...
      console.error(err)
      alert(err.message || '우선순위 타겟 설정에 실패했습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleResetOrLinkPassword = async (therapist: Therapist) => {
    if (!therapist.user_id && !therapist.email) {
      alert("비밀번호 설정(로그인 계정 생성)을 하려면 마사지사의 이메일 정보가 등록되어 있어야 합니다.\n\n먼저 수정(Edit) 버튼을 눌러 이메일을 등록해 주세요.");
      return;
    }

    if (therapist.user_id) {
      // 이미 로그인 계정이 있는 경우 -> 비밀번호 초기화
      const newPassword = prompt(`'${therapist.name}' 마사지사의 새 비밀번호를 입력해주세요.\n(미입력 시 'password123'으로 설정됩니다.)`)
      ...
      try {
        ...
        setSuccessMsg(`성공: '${therapist.name}' 마사지사의 비밀번호가 초기화되었습니다! (새 비밀번호: ${finalPassword})`)
      } catch (err: any) {
        console.error(err)
      ...
      const passwordInput = prompt(`'${therapist.name}' 마사지사의 로그인 계정을 생성합니다.\n사용할 초기 비밀번호를 입력해주세요.\n(미입력 시 'password123'으로 설정됩니다.)`)
      ...
      try {
        ...
        if (authError) throw authError
        if (!authData.user?.id) throw new Error('인증 계정 생성에 실패했습니다.')
        ...
        setSuccessMsg(`성공: '${therapist.name}' 마사지사의 로그인 계정(이메일: ${therapist.email})이 생성되었습니다! (초기 비번: ${finalPassword})`)
        onRefresh()
        refreshUsers()
      } catch (err: any) {
        console.error(err)
        setErrorMsg(err.message || '로그인 계정 생성 도중 오류가 발생했습니다.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleUpdateTherapist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`성공: '${editName}' 마사지사의 정보가 수정되었습니다.`)
      ...
      console.error(err)
      setErrorMsg(err.message || '마사지사 정보 수정 도중 오류가 발생했습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteTherapist = async (id: number, name: string) => {
    if (!confirm(`마사지사 '${name}' 직원을 정말로 삭제하시겠습니까?\n(해당 직원의 과거 예약 내역은 배정 마사지사가 없음 처리로 전환됩니다.)`)) return
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1 rounded-2xl border border-slate-800 bg-slate-900/40 p-5 h-fit">
        <h3 className="text-sm font-bold tracking-tight text-slate-200 mb-4 flex items-center gap-1.5 uppercase">
          <UserPlus className="w-4 h-4 text-indigo-500" /> 신규 마사지사 등록
        </h3>
        <form onSubmit={handleAddTherapist} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="예: 박안마"
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:border-indigo-500/80 transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="예: therapist1@jjimjil.com"
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-100 placeholder-slate-650 focus:outline-none focus:border-indigo-500/80 transition-colors"
            />
          </div>
      ...
      <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
        <h3 className="text-sm font-bold tracking-tight text-slate-200 mb-4">
          근무 마사지사 관리 ({therapists.length}명)
        </h3>
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-850 bg-slate-950/20">
          <table className="min-w-full divide-y divide-slate-850">
            <thead className="bg-slate-950/50 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-left">
              <tr>
                <th className="px-4 py-3">마사지사 정보</th>
                <th className="px-4 py-3">이메일</th>
                <th className="px-4 py-3">연락처</th>
                <th className="px-4 py-3">상태</th>
                <th className="px-4 py-3 text-center">고급 전담 지정</th>
                  ...
                  <tr key={therapist.id} className="hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {therapist.is_active ? (
                          <>
                            <CheckCircle2 className="w-3 h-3" /> 근무 중
                          </>
                        ) : (
                          <>
                            <XCircle className="w-3 h-3" /> 휴무
                          </>
                        )}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-center">
                      ...
                      >
                        {therapist.is_premium_target ? (
                          <>
                            <Star className="w-3 h-3 fill-current text-amber-500" /> 고급 전담
                          </>
                        ) : (
                          <>고급 전담 지정</>
                        )}
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title={therapist.user_id ? "비밀번호 초기화" : "로그인 계정 생성 (연동)"}
                        >
                          <Key className="w-4 h-4" />
                        </button>
                        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 relative">
            ...
            <h3 className="text-base font-bold text-slate-200 mb-6 flex items-center gap-1.5">
              <Edit className="w-4 h-4 text-indigo-400" /> 마사지사 정보 수정
            </h3>
